...
async function main() {
  ...
  console.log('=== 验证指标 ===');
  const expectedMajors = ['中医学', '中药学', '中药制药', '针灸推拿学', '中西医临床医学', '护理学', '生物工程', '法学', '公共事业管理', '英语'];
  const allPresent = expectedMajors.every(name => result.some(r => r.major_name.includes(name)));
  console.log('专业是否全部存在:', allPresent ? '✅' : '❌');

  const allHavePersonCount = result.every(r => r.person_count !== undefined);
  console.log('人数是否全部有值:', allHavePersonCount ? '✅' : '❌');

  const allHaveScore = result.every(r => r.min_score !== undefined);
  console.log('分数是否全部有值:', allHaveScore ? '✅' : '❌');

  const allHaveRank = result.every(r => r.min_rank !== undefined);
  console.log('位次是否全部有值:', allHaveRank ? '✅' : '❌');

  const hasBatchLineDiff = result.some(r => r.person_count && r.person_count > 100);
  console.log('人数是否混入批次线差:', hasBatchLineDiff ? '❌' : '✅');

  const allValid = allPresent && allHavePersonCount && allHaveScore && allHaveRank && !hasBatchLineDiff;
  console.log('\n=== 整体验证结果:', allValid ? '✅ 通过' : '❌ 未通过 ===');

  await browser.close();

  if (allValid) {
    console.log('\n主采集脚本提取逻辑验证通过，可以启动采集任务。');
  } else {
    console.log('\n主采集脚本提取逻辑验证未通过，请检查问题。');
    process.exit(1);
  }
}
