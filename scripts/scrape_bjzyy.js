...
const PROVINCES = [
  { name: '海南', outputDir: 'hainan_scores' },
  { name: '天津', outputDir: 'tianjin_scores' },
];
...
async function main() {
  ...
  for (const prov of PROVINCES) {
    console.log('\n🌏 ===== 采集 [' + prov.name + '] 数据 =====');
    await selectDropdown(page, containerIdx, 0, prov.name, '省份');

    for (const year of YEARS) {
      console.log('📅 [' + prov.name + '] ' + year + '年');
      const yearSelected = await selectDropdown(page, containerIdx, 1, year, '年份');
      if (!yearSelected) {
      ...
      await selectBatch(page, containerIdx, 2, '本科批');
      await page.waitForTimeout(2000);

      const result = await extractData(page);

      if (result.length === 0) {
        console.log('    ⏭ [' + prov.name + '] ' + year + '年无数据，跳过');
        continue;
      }

      const outputDir = path.join(__dirname, '..', 'data', prov.outputDir);
      if (!fs.existsSync(outputDir)) {
        fs.mkdirSync(outputDir, { recursive: true });
      }

      const outputFile = path.join(outputDir, schoolName + '_' + year + '_专业分数线.json');
      const finalResults = result.map(r => ({
        year: parseInt(year),
        school_name: schoolName,
        major_name: r.major_name || '',
        major_group: r.major_group || '',
        min_score: r.min_score === undefined ? null : r.min_score,
        min_rank: r.min_rank === undefined ? null : r.min_rank,
        person_count: r.person_count === undefined ? null : r.person_count,
        batch: r.batch || '本科批',
        major_description: r.major_description || '',
        subject_requirement: r.subject_requirement || '',
        province: prov.name,
      }));

      fs.writeFileSync(outputFile, JSON.stringify(finalResults, null, 2), 'utf-8');
      console.log('    ✓ 已保存: ' + outputFile + ' (' + finalResults.length + '条)');
    }
  }
...
async function selectBatch(page, containerIdx, buttonIdx, targetText) {
  try {
    const container = page.locator('.select-tabs-overflow').nth(containerIdx);
    const btn = container.locator('button.select-tabs-tab').nth(buttonIdx);
    ...
    if (clicked.found) {
      console.log('    ✓ 批次选择成功: ' + targetText);
      return true;
    } else {
      console.log('    ⚠ 批次未找到选项: ' + targetText);
      return false;
    }
  } catch (e) {
    console.log('    ✗ 批次选择失败: ' + e.message.substring(0, 50));
    return false;
  }
}
...
async function extractData(page) {
  return await page.evaluate(() => {
    ...
    for (const card of cards) {
      ...
      const cardResults = extractFromLines(lines);
