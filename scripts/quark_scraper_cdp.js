...
const COMPLETED_FILE = path.join(OUTPUT_DIR, 'completed_schools.json');
...
const EDGE_DEBUG_PORT = 9222;
const BATCH_SCRAPE = false;
const MAX_SCHOOLS = 0;
const DELAY_BETWEEN_SCHOOLS = 3000;
const DELAY_BETWEEN_YEARS = 2000;
...
function saveProgress(currentIndex, schools) {
  const progress = {
    currentIndex,
...
async function connectToBrowser() {
  ...
  try {
    ...
    console.log(`✅ 浏览器启动成功！检测到 ${contexts.length} 个浏览器上下文`);
    return browser;
  } catch (e) {
...
async function findQuarkPage(browser) {
  console.log('正在查找夸克高考页面...');
  ...
  for (const context of contexts) {
    ...
    for (const page of pages) {
      ...
      if (url.includes('quark.cn') && url.includes('gaokao')) {
        console.log(`✅ 找到夸克高考页面: ${title}`);
        return page;
      }
    }
  }
  
  console.log('⚠️ 未找到夸克高考页面');
  return null;
}
...
async function scrapeSchool(page, schoolName) {
  ...
  for (const year of YEARS) {
    ...
    await page.waitForTimeout(DELAY_BETWEEN_YEARS);
  }
...
async function main() {
  ...
  console.log('夸克高考专业分数线自动抓取工具 (CDP连接Edge浏览器)');
  console.log('='.repeat(70));
  ...
  console.log('\n正在访问夸克高考页面...');
  const testUrl = buildSchoolUrl('海南大学', 2025);
  ...
  let successCount = 0;
  let failCount = 0;
  const completedSchools = loadCompletedSchools();
  const startIndex = loadProgress();

  console.log(`\n已完成院校: ${completedSchools.size} 所`);
  console.log(`上次进度: 从第 ${startIndex + 1} 所开始`);

  try {
    if (!BATCH_SCRAPE) {
      const testSchool = '海南大学';
      console.log(`\n测试院校: ${testSchool}`);
      ...
      if (records.length > 0) {
        ...
        console.log('\n✅ 测试成功！');
        console.log('如需批量抓取所有院校，请设置 BATCH_SCRAPE = true');
      } else {
        failCount++;
        console.log('\n❌ 测试抓取失败');
        console.log('请检查浏览器中是否正确显示了专业分数线数据');
      }
      ...
      const totalSchools = MAX_SCHOOLS > 0 ? Math.min(MAX_SCHOOLS, schools.length) : schools.length;
      const schoolsToProcess = schools.slice(startIndex, startIndex + totalSchools);

      console.log('\n' + '='.repeat(70));
      console.log('🚀 开始批量抓取专业分数线数据');
      console.log(`📊 共 ${schoolsToProcess.length} 所院校, ${YEARS.length} 个年份`);
      console.log(`⏱ 预计时间: ${Math.ceil(schoolsToProcess.length * (YEARS.length * 10 + 5) / 60)} 分钟`);
      console.log('='.repeat(70));

      const startTime = Date.now();

      for (let i = 0; i < schoolsToProcess.length; i++) {
        const schoolIndex = startIndex + i;
        ...
        try {
          const records = await scrapeSchool(page, school);
        ...
        saveProgress(schoolIndex + 1, schools);
        saveCompletedSchools(Array.from(completedSchools));

        if ((i + 1) % SAVE_INTERVAL === 0 || i === schoolsToProcess.length - 1) {
          const allFile = path.join(OUTPUT_DIR, 'major_scores_all.json');
          fs.writeFileSync(allFile, JSON.stringify(allRecords, null, 2), 'utf-8');
          console.log(`\n💾 已保存进度，共 ${allRecords.length} 条数据`);
        }

        const elapsed = Math.floor((Date.now() - startTime) / 1000);
        const progress = ((i + 1) / schoolsToProcess.length * 100).toFixed(1);
        console.log(`\n📈 进度: ${schoolIndex + 1}/${schools.length} (${progress}%) | 成功: ${successCount} | 失败: ${failCount} | 用时 ${Math.floor(elapsed/60)}分${elapsed%60}秒`);

        await page.waitForTimeout(DELAY_BETWEEN_SCHOOLS);
      }
