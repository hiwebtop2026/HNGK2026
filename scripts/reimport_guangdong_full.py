...
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'guangdong_major_scores_all.json')
...
def delete_old_data():
    print("\n🔄 删除旧的广东数据...")
    try:
        result = supabase.table('major_scores').delete().eq('province', '广东').execute()
...
def verify_import(schools_found, schools_with_valid_data):
    ...
    try:
        result = supabase.table('major_scores').select('*').eq('province', '广东').execute()
        records = result.data
        
        print(f"\n数据库中广东记录数: {len(records)}")
        
        db_schools = set(r['school_name'] for r in records if r['school_name'])
        print(f"数据库中广东学校数: {len(db_schools)}")
