...
async function main() {
  ...
  console.log('\n=== 尝试选择省份: 海南 ===');
  await clickAndSelect(page, 4, 0, '海南', '省份');
  await page.waitForTimeout(3000);

  console.log('\n=== 尝试选择年份: 2023 ===');
  await clickAndSelect(page, 4, 1, '2023', '年份');
...
async function clickAndSelect(page, containerIdx, buttonIdx, targetText, buttonType) {
  ...
  for (const opt of options) {
    console.log('    "' + opt.text + '" (可见:' + opt.visible + ')');
  }
