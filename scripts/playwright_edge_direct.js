...
async function setupBrowser() {
  ...
  const context = await chromium.launchPersistentContext(userDataDir, {
    ...
    headless: false,
    viewport: null,
    userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/150.0.0.0 Safari/537.36 Edg/150.0.4078.48',
    locale: 'zh-CN',
...
async function goToSchoolPage(page, schoolName, year, province) {
  ...
  try {
    ...
    console.log('  ❌ 访问失败:', schoolName, e.message);
    return false;
  }
}
...
async function extractFromDOM(page, schoolName, year, province) {
  try {
    return await page.evaluate(function(args) {
      ...
      if (results.length === 0) {
        const items = document.querySelectorAll('[class*="content-List-li"], [class*="major-item"], [class*="score-item"], [class*="list-item"], div[role="listitem"]');
