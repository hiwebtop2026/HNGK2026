...
async function updateSchoolLevels() {
    console.log('='.repeat(70));
    console.log('🔧 更新数据库中的院校层次和性质字段');
    console.log('='.repeat(70));

    const schoolLevelMap = new Map();
    const schoolNatureMap = new Map();

    const DATA_DIRS = [
        join(__dirname, '..', 'data', 'hainan_scores'),
        join(__dirname, '..', 'data', 'tianjin_scores'),
    ];
        ...
        for (const file of files) {
            try {
                ...
                for (const item of data) {
                    ...
                    if (!schoolLevelMap.has(schoolName)) {
                        schoolLevelMap.set(schoolName, getSchoolLevel(schoolName));
                    }
                    if (!schoolNatureMap.has(schoolName)) {
                        schoolNatureMap.set(schoolName, getSchoolNature(schoolName));
                    }
                }
            } catch (e) {}
        }
    ...
    console.log(`\n📊 共处理 ${schoolLevelMap.size} 所院校`);

    const schools = Array.from(schoolLevelMap.keys());
    const batchSize = 50;
    let updatedCount = 0;
    ...
    for (let i = 0; i < schools.length; i += batchSize) {
        ...
        for (const schoolName of batch) {
            try {
                const level = schoolLevelMap.get(schoolName);
                const nature = schoolNatureMap.get(schoolName);
        ...
        if (batchNum % 10 === 0 || batchNum === totalBatches) {
            console.log(`   进度: ${batchNum}/${totalBatches} 批次, 更新 ${updatedCount} 所院校`);
        }
    }

    console.log(`\n✅ 更新完成: ${updatedCount} 所院校的层次和性质字段已更新`);
...
async function verifyUpdate() {
    ...
    for (const province of provinces) {
        const { data: sampleData, error } = await supabase
            .from('major_scores')
            .select('school_name, level, nature')
        ...
        if (sampleData && sampleData.length > 0) {
            for (const item of sampleData) {
                console.log(`     ${item.school_name}: 层次=${item.level}, 性质=${item.nature}`);
            }
        }
    }
    ...
    if (!error && stats) {
        ...
        for (const item of stats) {
            ...
            natureCounts[item.nature] = (natureCounts[item.nature] || 0) + 1;
        }

        console.log('\n📊 层次分布:');
        for (const [level, count] of Object.entries(levelCounts)) {
            console.log(`   ${level}: ${count} 条记录`);
        }

        console.log('\n📊 性质分布:');
        for (const [nature, count] of Object.entries(natureCounts)) {
            console.log(`   ${nature}: ${count} 条记录`);
        }
    }
...
updateSchoolLevels().catch(console.error);