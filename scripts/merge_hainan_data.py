...
def generate_sql(data: List[Dict]) -> str:
    ...
    for item in data:
        ...
        row = f"""('{school_name_escaped}', '海南', {year}, '{major_name_escaped}', 
    ...
    lines.append("SELECT school_name, COUNT(*) as majors FROM major_scores WHERE province = '海南' ")
    ...
    lines.append("WHERE province = '海南' AND major_description IS NOT NULL AND major_description != '';")
...
def import_to_supabase(data: List[Dict]) -> bool:
    ...
    print("\n⏳ 清理旧的海南数据...")
    try:
        supabase.table('major_scores').delete().eq('province', '海南').execute()
        print("✅ 旧数据清理完成")
    except Exception as e:
        print(f"❌ 清理失败: {e}")
        return False
    ...
    print("\n⏳ 验证导入结果...")
    result = supabase.table('major_scores').select('province', 'year').eq('province', '海南').execute()
    if result.data:
        year_counts = {}
        ...
        for year in sorted(year_counts.keys()):
            print(f"   海南 {year}年: {year_counts[year]}条")
