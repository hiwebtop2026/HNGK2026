...
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'hainan_scores')
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'hainan_major_scores_all.json')
...
def delete_old_data():
    print("\n🔄 删除旧的海南数据...")
    try:
        result = supabase.table('major_scores').delete().eq('province', '海南').execute()
...
def verify_import(schools_found, schools_with_valid_data):
    ...
    try:
        result = supabase.table('major_scores').select('*').eq('province', '海南').execute()
        records = result.data
        
        print(f"\n数据库中海南记录数: {len(records)}")
        
        db_schools = set(r['school_name'] for r in records if r['school_name'])
        print(f"数据库中海南学校数: {len(db_schools)}")
