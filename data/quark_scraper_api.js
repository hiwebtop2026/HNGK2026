(function() {
      ...
      try {
        ...
        const response = await fetch(url, {
          ...
          headers: {
            ...
            'Referer': 'https://vt.quark.cn/blm/gaokao-college-794/tab',
        ...
        console.log(`    📥 响应大小: ${html.length} 字符`);
        
        return extractFromHtml(html, schoolName, year);
      } catch (e) {
        console.log(`    ⚠ 请求失败 (${retry + 1}/${CONFIG.maxRetry}): ${e.message}`);
  ...
  function extractFromHtml(html, schoolName, year) {
    try {
  ...
  async function scrapeSchool(schoolName) {
    ...
    for (const year of CONFIG.years) {
      console.log(`  📅 ${year}年`);
      const records = await fetchMajorData(schoolName, year);
  ...
  function saveProgress() {
    try {
      localStorage.setItem('quark_scraper_api_progress', JSON.stringify({
        currentIndex,
  ...
  function loadProgress() {
    try {
      const saved = localStorage.getItem('quark_scraper_api_progress');
      if (saved) {
  ...
  function testCurrent() {
    ...
    const data = extractFromHtml(document.documentElement.outerHTML, schoolName, year);
  ...
  function clearProgress() {
    localStorage.removeItem('quark_scraper_api_progress');
    currentIndex = 0;
  ...
  console.log('\n' + '='.repeat(60));
  console.log('%c🎓 夸克高考专业分数线抓取工具 (API版)', 'color:#3b82f6;font-size:20px;font-weight:bold');
  console.log('='.repeat(60));
  ...
  window.QuarkScraper = {
    ...
    resume: () => {
      const saved = loadProgress();
      const idx = saved ? saved.currentIndex || 0 : 0;
